...
If you have real EIS measurements from an electrode, this notebook lets you run the detector on your own data. It supports two formats: a folder of CSV files (one per measurement, with columns `frequency_hz`, `re_z_ohm`, `neg_im_z_ohm`) or a single CSV of pre-fitted Rct values over time. Edit the configuration cell at the top to point to your files, then run the notebook top to bottom. If you get results - whether the detector works or not - please open an issue or reach out on X [@02iTAG](https://x.com/02iTAG).
